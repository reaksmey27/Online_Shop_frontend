  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-500 text-sm">
        ...
        <template v-else> Enter a search term above to find products </template>
      </p>
    </div>
    ...
    <template v-else>
      ...
      >
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <span
            class="text-xs font-bold text-gray-500 uppercase tracking-wider mr-1"
          >
            Filter:
          </span>
          <button
        ...
        <select
          v-model="sortBy"
          @change="fetchResults"
          class="text-sm border border-gray-200 rounded-xl px-3 py-2 outline-none focus:border-blue-500 bg-white"
        >
      ...
      >
        <div
          ...
          :key="product.id"
          class="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-blue-100 transition-all group flex flex-col"
        >
          <!-- Image -->
          <router-link
          ...
          >
            ...
            >
              <span
                class="bg-gray-800 text-white text-xs font-bold px-3 py-1 rounded-full"
              >
                Out of Stock
              </span>
            </div>
          ...
          <div class="p-4 flex-1 flex flex-col justify-between">
            <div>
              <p
                class="text-[10px] text-blue-500 font-black tracking-wider uppercase mb-1"
              >
              ...
              <router-link
                :to="`/products/${product.id}`"
                class="font-bold text-gray-800 text-sm hover:text-blue-600 line-clamp-2 block leading-snug transition-colors"
              >
                <span v-html="highlightQuery(product.name)" />
              </router-link>
            </div>
            <div class="mt-3 flex items-center justify-between">
              ...
              <button
                ...
                type="button"
                class="text-white text-xs font-bold px-3 py-2 rounded-xl transition-all flex items-center gap-1.5 active:scale-[0.97]"
                :class="
                  product.stock <= 0
      ...
      >
        ...
        <div class="flex gap-3 justify-center mt-6">
          ...
          <router-link
            to="/categories"
            class="border border-gray-200 text-gray-700 hover:bg-gray-50 px-5 py-2.5 rounded-xl text-sm font-medium transition-colors"
          >
            View Categories
          </router-link>
        </div>
...
function highlightQuery(text) {
  ...
  return text.replace(
    ...
    (match) =>
      `<mark class="bg-yellow-100 text-yellow-800 rounded px-0.5">${match}</mark>`,
  );
}
